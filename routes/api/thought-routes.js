...
router.post('/:thoughtId/reactions/:reactionId', async (req, res) => {
    try {
    const result = await User.findOneAndUpdate({ _id: req.params.id }, {$addToSet:{reactions: req.params.reactionId}},{new:true})
    res.status(200).json(result)
    } catch (err) {
        res.status(500).json({error: 'Something went wrong'})
    }
...
router.delete('/:thoughtId/reactions/:reactionId', async (req, res) => {
    try {
    const result = await User.findOneAndUpdate({ _id: req.params.id }, {$pull:{friends: req.params.reactionId}},{new:true})
    res.status(200).json(result)
    } catch (err) {
        res.status(500).json({error: 'Something went wrong'})
    }
